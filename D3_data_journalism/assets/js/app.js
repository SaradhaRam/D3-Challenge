...
function updateToolTip(chosenXAxis, chosenYAxis, circlesGroup, textGroup) {
    ...
    var toolTip = d3.tip()
        ...
        .html(function(d) {
            if (chosenXAxis === "age") {
                // yAxis tooltip labels formated as %.
                // Age without format for xAxis.
                return (`${d.state}<hr>${xlabel} ${d[chosenXAxis]}<br>${ylabel}${d[chosenYAxis]}%`);
            } else if (chosenXAxis !== "poverty" && chosenXAxis !== "age") {
                // Income in dollars for xAxis.
                return (`${d.state}<hr>${xlabel}$${d[chosenXAxis]}<br>${ylabel}${d[chosenYAxis]}%`);
            } else {
                // Poverty as percentage for xAxis.
                return (`${d.state}<hr>${xlabel}${d[chosenXAxis]}%<br>${ylabel}${d[chosenYAxis]}%`);
            }
        });
...
d3.csv("assets/data/data.csv").then(function(healthData, err) {
    ...
    var xLinearScale = xScale(healthData, chosenXAxis, chartWidth);
    var yLinearScale = yScale(healthData, chosenYAxis, chartHeight);
    ...
    xLabelsGroup.selectAll("text")
        .on("click", function() {
            ...
            // Update xLinearScale.
            xLinearScale = xScale(healthData, chosenXAxis, chartWidth);
            // Render xAxis.
    ...
    yLabelsGroup.selectAll("text")
        .on("click", function() {
            ...
            // Update yLinearScale.
            yLinearScale = yScale(healthData, chosenYAxis, chartHeight);
            // Update yAxis.
